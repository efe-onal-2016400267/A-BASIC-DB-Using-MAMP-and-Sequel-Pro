...
if (isset($_GET["id"])) {
  try {
    ...
    $sql2 = "DELETE FROM topic_paper WHERE topic_paper.title IN (
    SELECT p.title FROM papers AS p WHERE p.title = topic_paper.title AND p.id = $id)";
    $statement2 = $connection->prepare($sql2);
    $statement2->bindValue(':title', $id);
    $statement2->execute();

    $sql1 = "DELETE FROM paper_author WHERE paper_author.title IN (
    SELECT p.title FROM papers AS p WHERE p.title = paper_author.title AND p.id = $id)";
    $statement1 = $connection->prepare($sql1);
    $statement1->bindValue(':title', $id);
    $statement1->execute();
    ...
    #$sql2 = "DELETE FROM topic_paper WHERE title = :title";
    #$statement2 = $connection->prepare($sql2);
    #$statement2->bindValue(':title', $,d);
    #$statement2->execute();

    $success = "Paper successfully deleted";
 ...
 <table>
  ...
  <?php foreach ($papers_array as $row) : ?>
    <tr>
      ...
      <td><a href="delete_paper.php?id=<?php echo escape($row["id"]); ?>">Delete</a></td>
  </tr>
  <?php endforeach; ?>
  </tbody>
</table>
